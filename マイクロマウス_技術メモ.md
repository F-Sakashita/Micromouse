...
- PID制御の微分項に対し，理想的な微分を実装することはできない（インプロパー，つまり未来項は計算できないため）
- そこで，微分器にローパスフィルタを付与した不完全微分が用いられる．

$$
y_d(s) = \frac{1}{1 + T_ps}K_d \dot{e}(s) \\ T_p : カットオフ周波数, K_d:Dゲイン
$$

- ラプラス領域→離散化(z変換)→差分方程式にして実装する
- 離散化の手法は一般的に
  - 0次ホールド
  - 双一次変換

​	ref1 : Pythonによる制御工学入門, p151

​	ref2 :  シミュレーションで学ぶ自動制御入門, p62 

### 位置型と速度型

- PID制御の実装方法には位置型と速度型がある

- 位置型は一般的なPIDで，各ゲイン項の合計値が出力値となる(全値出力形演算方式)
  $$
  y_k = K_p e_k + K_i \Delta t \sum_{0}^{n-1}{e_k} + \frac{K_d}{\Delta t} (e_k - e_{k-1})\\ \Delta t : サンプリング時間
  $$

- 速度型は各ゲイン項の合計値が出力値の変化量になる(インクリメンタル形演算方式)

- 速度型は，位置型のように偏差が残り続けると積分項が累積して出力が発散するということがない．
  $$
  y_k = y_{k-1} + \Delta y_k \\
  \Delta y_k = K_p (e_k - e_{k-1}) + K_i \Delta t e_k + \frac{K_d}{\Delta t}(e_k - 2e_{k-1} + e_{k-2})
  $$
...
3. WallDetectTask
   ...
   壁を検出して，0現在のロボットの周辺○ｍｍ以内力する（とりあえず最初は前方と左右）
...
4. MainTask
   ...
   - 自己位置推定
     ...
     - ありえ9ない座標や姿勢の場合はエラー状態に落とす（速度制御や軌道制御のタスクは一時停止する）
   - 軌道生成
     - 時間ベースで更新されていく
     - 探索時はほぼ一定速度and足立式
     - 探索後はマップ情報をもとに作成
